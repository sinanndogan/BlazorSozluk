...
namespace BlazorSozluk.Api.Application.Features.Queries.GetEntries
{
    public class GetEntriesQueryHandler : IRequestHandler<GetEntriesQuery, List<GetEntriesViewModel>>
    {
        ...
        {
            //query değişkeni burada IQueryable türünde bir nesne oldu 
            ...
            //Bütün kayıtlar rastgele sıralandıktan sonra getir 
          query= query.Include(a => a.EntryComments).OrderBy(i => Guid.NewGuid()).Take(request.Count);
